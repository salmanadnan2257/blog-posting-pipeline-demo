...
\begin{tikzpicture}[node distance=8mm]
  ...
  \node[lbl, right=3mm of app, text width=30mm] {no \code{fetch}, no\\ \code{XMLHttpRequest},\\ anywhere};
\end{tikzpicture}}
...
\begin{tikzpicture}[node distance=7mm]
  ...
  \draw[dflow] (presetf.west) to[out=180,in=0] node[lbl, above] {modeledAfter\\ resolves to} (realf.east);
  \draw[dflow] (presetf.east) to[out=0,in=180] node[lbl, above] {id\\ selects} (runf.west);
\end{tikzpicture}}
...
\begin{tikzpicture}[node distance=4mm]
  ...
  \node[extbox, above=9mm of s2, text width=30mm] (res) {\scriptsize AEO/SEO research\\ (Perplexity)\\ \scriptsize \textit{unnumbered stage}};
...
\begin{tikzpicture}[node distance=9mm]
  ...
  \draw[dflow] (done.north) |- ++(0,9mm) -| node[lbl, pos=0.25, above] {click ``Reset''} (idle.north);
\end{tikzpicture}}
...
\begin{tikzpicture}[node distance=10mm]
  ...
  \draw[flow] (again) -- node[lbl, above] {yes: clear both\\ handles, restart} ($(live.south)+(0,-5mm)$);
